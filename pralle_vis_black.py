...
def draw():
    ...
    # 阶段 3: 括号展示阶段
        ...
        for i, token in enumerate(tokens):
            x = margin + i * spacing
            py5.fill(80); py5.text_size(20); py5.text(token, x, y_tokens)
            target_x = start_target_x + i * num_spacing
            py5.fill(150, 255, 150); py5.text_size(24); py5.text(output_numbers[i], target_x, y_numbers)
            
        bracket_progress = (cycle_frame - t2) / (phase3_duration * 0.5)
        bracket_alpha = py5.constrain(bracket_progress * 255, 0, 255)
        py5.fill(255, bracket_alpha); py5.text_size(48)
    ...
    # 阶段 4: 上移淡出阶段
        ...
        for i, token in enumerate(tokens):
            x = margin + i * spacing
            py5.fill(80, old_token_alpha); py5.text_size(20); py5.text(token, x, y_tokens)
            target_x = start_target_x + i * num_spacing
            py5.fill(150, 255, 150); py5.text_size(24); py5.text(output_numbers[i], target_x, current_y)
        py5.fill(255, bracket_alpha); py5.text_size(48)
    ...
    # 阶段 5: 重新展开阶段
        ...
        for i, token in enumerate(tokens):
            ...
            py5.fill(c_r, c_g, c_b); t_size = py5.lerp(24, 20, progress)
